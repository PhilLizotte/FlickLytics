...
"amazing", "best", "blast", "brilliant", "captivating",
     ...
     * @param id The id of the movie or tv show UNUSED
     ...
     */
    public CompletionStage<List<String>> fetchReviewsList(String kind, int id) {
     ...
     * @return A CompletionStage of the complete {@link models.domain.Review Review} object
     * 
     * Since each page of reviews contains a maximum of 20 reviews, we need to do three API calls
     * to fetch the first three pages of reviews.
     */
     ...
     * 
     * This is the core of the sentiment service. The method parses all the reviews and determines the sentiment of each, as well as the overall sentiment. 
     */
    public Review extractSentiment(List<String> reviewsList, int id) {
        ...
        // This should never happen. The only way this condition will ever be triggered is if tmdb changes
        // the return of the review query, making it so that each page contains strictly more than 25 reviews
        ...
        {
            // INFO:: this stream is the same as the FOR loop below, except that negations are not calculated. 
            // List<String> sents = reviewsList.stream()
            //     .map(r -> {
            //         // List<String> revs = Arrays.asList(r.toLowerCase().replaceAll("[^a-zé\\-\\s]", "").split(" "));

            //         int points = Arrays.asList(r.toLowerCase().replaceAll("[^a-zé\\-\\s]", "").split(" ")).stream()
            //             .mapToInt(w -> {
            ...
            // We know that there is at least 1 review because if there are no reviews then neutral is automatically returned. 
            // Problems happen if the threshold is 0, namely, everything will become positive. 
